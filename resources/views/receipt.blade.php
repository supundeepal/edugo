    ...
    <div class="receipt-paper">
        ...
        <div class="r-details">
            <div class="r-row">
                ...
                <span class="r-value">#{{ str_pad($payment->id, 5, '0', STR_PAD_LEFT) }}</span>
            </div>
            <div class="r-row">
                <span class="r-label">Date:</span> 
                <span class="r-value">{{ $payment->created_at->format('Y-m-d h:i A') }}</span>
            </div>
            ...
            @if($payment->student)
                <div class="r-row">
                    ...
                    <span class="r-value">{{ $payment->student->student_name }}</span>
                </div>
                <div class="r-row">
                    <span class="r-label">Index No:</span> 
                    <span class="r-value">{{ $payment->student->card_number }}</span>
                </div>
            @else
                <div class="r-row">
                    <span class="r-label">Student:</span> 
            ...
            @if($payment->course)
                <div class="r-row">
                    ...
                    <span class="r-value">{{ $payment->course->course_name }}</span>
                </div>
            @endif
            
            <div class="r-row">
                <span class="r-label">Type:</span> 
                <span class="r-value">{{ $payment->month == 'Daily' ? 'Day Payment' : 'Monthly ('.$payment->month.')' }}</span>
            </div>
        </div>

        <div class="r-amount">
            <span>TOTAL PAID</span>
            <h1>Rs. {{ number_format($payment->amount, 2) }}</h1>
        </div>
